...
try:
    ...
    # 2. SELF-CLEANING MECHANIC: Automatically strips out accidental text wrappers or equal signs
    clean_key = raw_secret.replace("GOOGLE_API_KEY", "").replace("=", "").replace('"', "").replace("'", "").strip()
    
    # 3. Configure the official library core using the cleaned key parameters
    ...
    if user_question:
        if api_ready and ai_model is not None:
            with st.spinner("Gemini is analyzing biomaterial properties..."):
                ...
                try:
                    # Direct client instruction using the native official library object
